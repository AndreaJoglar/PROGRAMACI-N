...
public class Libreria {
	...
	/**
	 * Funcion que suma los valores de la diagonal secundaria de una matriz
	 * @param m matriz de enteros
	 * @return entero
	 */
	...
	/**
	 * Funcion que determina si una matriz es magica
	 * @param m matriz de enteros
	 * @return boolean
	 */
	public static boolean matrizMagica (int [][]m) {
		for (int i = 0; i < m.length; i++) {
			for (int j = 0; j < m[0].length; j++) {
				if(sumaFila(m, i)==sumaColumna(m, j)){
					if(sumaColumna(m, j)==sumaDiagonalPrincipal(m)) {
						if (sumaDiagonalPrincipal(m)==sumaDiagonalSecundaria(m)) {
							return true;
						}
					}
				}
