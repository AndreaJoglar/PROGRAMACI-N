...
public class menuFestival {
	...
	public static void main(String[] args) {
		...
		while (opcion != 0) {
			...
			case 6: {
				System.out.println("Indica el ID del artista para actualizar el caché: ");
				String id = teclado.next();
				if (festival.buscarArtista(id) != null) {
					Artista a = festival.buscarArtista(id);
					System.out.println("Artista encontrado: " + "\n\t" + a.toString());
					System.out.println("Indica el nuevo caché: ");
					double nuevoCache = teclado.nextDouble();
					festival.actualizarNuevoCache(nuevoCache, a);
				}
				else {
					System.out.println("Artista no encontrado.");
				}
	...
	private static boolean disponeDeTecnico() {
		...
		System.out.println("El grupo necesita la contratación de técnico de sonido: SI / NO");
		String respuesta = teclado.next();
		
		while (respuesta.charAt(0) != 'S' && respuesta.charAt(0) != 's'
				&& respuesta.charAt(0) != 'N' && respuesta.charAt(0) != 'n') {
			System.out.println("ERROR. Por favor, responda SI o NO:");
	...
	private static int menu() {
		...
		System.out.println("\t6- Actualizar caché del artista: ");
